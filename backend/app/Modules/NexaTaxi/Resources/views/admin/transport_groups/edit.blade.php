...
@section('title', $group->name . ' bewerken')

@section('content')
<div class="kt-container-fixed min-w-0">
    <div class="pb-7.5">
        <h1 class="text-xl font-medium leading-none text-mono">{{ $group->name }} bewerken</h1>
    ...
    <form method="POST" action="{{ route('admin.taxi.transport_groups.update', [$customer->id, $contract->id, $group->id]) }}">
        @csrf
        @method('PUT')
            ...
            <div class="kt-card w-full min-w-0">
                ...
                <div class="kt-card-content p-0">
                    <div class="px-3 sm:px-5 pb-3 min-w-0">
                        @include('taxi::admin.transport_groups.partials.form-fields', ['group' => $group])
                    </div>
                </div>
            </div>
        </div>
