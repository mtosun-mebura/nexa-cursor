...
class VacancyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('skillmatching::admin.vacancies.index', compact('vacancies', 'statusStats', 'branches', 'companies'));
    }

    public function create()
    {
        if (!auth()->user()->hasRole('super-admin') && !auth()->user()->can('create-vacancies') && !auth()->user()->can('skillmatching.vacancies.create')) {
        ...
        return view('skillmatching::admin.vacancies.create', compact('companies', 'branches', 'users', 'employmentTypes', 'workingHours', 'statuses', 'salaryBrutoPerMaand', 'salaryZzpUurtarief', 'companyLocations', 'selectedCompany'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.skillmatching.vacancies.index')->with('success', 'Vacature succesvol aangemaakt.');
    }
    ...
    public function show(Vacancy $vacancy)
    {
        ...
        $matches = JobMatch::where('vacancy_id', $vacancy->id)
            ->with(['candidate'])
        ...
        return view('skillmatching::admin.vacancies.show', compact('vacancy', 'matches', 'applications'));
    }

    public function edit(Vacancy $vacancy)
    {
        if (!auth()->user()->hasRole('super-admin') && !auth()->user()->can('edit-vacancies') && !auth()->user()->can('skillmatching.vacancies.edit')) {
        ...
        return view('skillmatching::admin.vacancies.edit', compact('vacancy', 'companies', 'branches', 'users', 'employmentTypes', 'workingHours', 'statuses', 'salaryBrutoPerMaand', 'salaryZzpUurtarief'));
    }
    ...
    {
        if (!auth()->user()->hasRole('super-admin') && !auth()->user()->can('delete-vacancies') && !auth()->user()->can('skillmatching.vacancies.delete')) {
    ...
    public function showCandidate(Vacancy $vacancy, Candidate $candidate, Request $request)
    {
        ...
        $interviews = collect();
        if ($match) {
            $interviews = Interview::where('match_id', $match->id)
        ...
        return view('skillmatching::admin.vacancies.candidate', compact('vacancy', 'candidate', 'match', 'application', 'type', 'timeline', 'companyLocations', 'companyUsers', 'interviews', 'latestInterview', 'stageInstances', 'pipelineTemplate', 'availableTemplates', 'company'));
    }
    ...
    public function scheduleInterview(Vacancy $vacancy, Candidate $candidate, Request $request)
    {
        ...
        return redirect()->route('admin.skillmatching.vacancies.candidate', [
            'vacancy' => $vacancy->id,
