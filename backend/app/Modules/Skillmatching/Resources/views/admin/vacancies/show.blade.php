...
<div class="bg-center bg-cover bg-no-repeat hero-bg">
    ...
    <div class="kt-container-fixed">
        <div class="flex flex-col items-center gap-2 lg:gap-3.5 py-4 lg:pt-5 lg:pb-10">
            @if($vacancy->company && $vacancy->company->hasAdminLogo())
                @include('admin.partials.company-theme-logo', ['company' => $vacancy->company])
...
<div class="kt-container-fixed">
    <div class="flex items-center flex-wrap md:flex-nowrap lg:items-center justify-between gap-3 lg:gap-6 mb-5 lg:mb-10 mt-5">
        ...
        <div class="flex items-center gap-2.5">
            @can('edit-vacancies')
                <a href="{{ route('admin.skillmatching.vacancies.edit', $vacancy) }}" class="kt-btn kt-btn-primary">
                                        ...
                                        @if($contactName || $contactEmail || $contactPhone || $contactPhoto)
                                            <div class="flex items-start gap-3">
                                                @if($contactPhoto)
                                                    ...
                                                    @else
                                                        <img src="{{ route('admin.skillmatching.vacancies.contact-photo', $vacancy) }}" alt="Contactpersoon avatar" class="w-12 h-12 rounded-full object-cover border-2 border-input shrink-0 mt-0.5">
                                                    @endif
                                                @endif
                ...
                @if($matches->count() > 0)
                    <div class="grid grid-cols-1 gap-4">
                        @foreach($matches as $match)
                            ...
                            @endphp
                            <div class="kt-card hover:shadow-lg transition-shadow cursor-pointer" onclick="window.location.href='{{ route('admin.skillmatching.vacancies.candidate', ['vacancy' => $vacancy->id, 'candidate' => $candidate->id, 'type' => 'match', 'match_id' => $match->id]) }}'">
                                <div class="kt-card-content">
                ...
                @if($applications->count() > 0)
                    <div class="grid grid-cols-1 gap-4">
                        @foreach($applications as $application)
                            ...
                            @endphp
                            <div class="kt-card hover:shadow-lg transition-shadow cursor-pointer" onclick="window.location.href='{{ route('admin.skillmatching.vacancies.candidate', ['vacancy' => $vacancy->id, 'candidate' => $candidate->id, 'type' => 'application', 'application_id' => $application->id]) }}'">
                                <div class="kt-card-content">
                                    <div class="flex items-center justify-between gap-4">
                                        <div class="flex items-center gap-4 flex-1">
                                            @if($candidate->photo_blob)
