        ...
        try {
            ...
            if (notificationElement) {
                ...
                if (actionButtonsContainer && actionButtonsContainer.querySelector('button.accept-interview, button.decline-interview')) {
                    ...
                    // For candidates: if interview is scheduled later, it will show "Ingepland" after reload
                    // For now, show "Verzoek geaccepteerd" (will be updated after reload if interview_exists = true)
            ...
            if (notificationElementAfterReload) {
                ...
                if (reloadedNotification) {
                    ...
                    if (actionButtonsContainer) {
                        // Only update if there's actually a response - remove buttons completely if response exists
                        if (reloadedNotification.has_response || reloadedNotification.response_type) {
                            ...
                            // If no response, ensure buttons are shown (they should already be there)
                            // But if they're missing, don't add them here - let the normal rendering handle it
                        }
                    }
                }
            }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (footer) {
        ...
        if (markAllReadBtn) {
            ...
            markAllReadBtn.addEventListener('click', originalMarkReadHandler);
        }
    }
    ...
    document.querySelectorAll('[data-kt-drawer-toggle="#notifications_drawer"], .notification-icon-button').forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            setTimeout(() => {
                ...
                // Load notifications immediately when drawer opens (both admin and frontend)
                loadNotifications();
                
                // Start polling for new notifications (both admin and frontend)
                startNotificationPolling();
            }, 50);
        });
    });
