...
@section('title', $job->title . ($job->company ? ' - ' . $job->company->name : '') . ' | Nexa Skillmatching')
...
@section('content')
      ...
      <nav class="mb-6">
          <ol class="flex items-center space-x-2 text-sm text-muted dark:text-muted-dark">
              ...
              <li class="flex items-center">
                  ...
                  <span class="text-primary dark:text-primary-dark">{{ $job->title }}</span>
              </li>
          </ol>
      </nav>
    ...
    <div class="card p-6 mb-6">
        <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
            <div class="flex-1">
                ...
                @if($job->company && $job->company->is_intermediary)
                    <p class="text-lg text-muted dark:text-muted-dark mb-4">{{ $job->company->name }}</p>
                @endif
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        @if($job->company && $job->company->is_intermediary)
            <div class="card p-6">
                <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-4">Over {{ $job->company->name }}</h2>
                <div class="space-y-3">
                    ...
                    @if($job->company->website)
                        <div class="flex items-center gap-2 text-sm text-muted dark:text-muted-dark">
                            ...
                            <a href="{{ $job->company->website }}" target="_blank" class="text-primary dark:text-primary-dark hover:underline">
                                {{ $job->company->website }}
                            </a>
                        </div>
                    @endif
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  @foreach($relatedJobs as $related)
                      <div class="related-vacancy-card border border-gray-200 dark:border-gray-700 rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer"
                           data-href="{{ route('jobs.show', $related) }}"
                           role="link"
                           tabindex="0">
                          <h3 class="font-semibold text-gray-900 dark:text-white mb-2">{{ $related->title }}</h3>
                          
                          @if($related->company && $related->company->is_intermediary)
...
async function saveVacancy() {
    ...
    try {
        ...
        const response = await fetch('{{ route("favorites.toggle", $job) }}', {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            console.error('Favorites error response:', response.status, errorText.substring(0, 500));
            let serverMessage = '';
            try {
                const errData = JSON.parse(errorText);
                if (errData && errData.message) serverMessage = '\n\n' + errData.message;
            } catch (_) {
                if (errorText.trim().length > 0) serverMessage = '\n\n(Zie console voor serverresponse.)';
            }
        ...
        alert('Er is een fout opgetreden bij het opslaan van de vacature.' + (error.message ? '\n\n' + error.message : ''));
    }
    @endauth
}
...
document.addEventListener('DOMContentLoaded', async function() {
    try {
        const response = await fetch('{{ route("favorites.check", $job) }}');
        const data = await response.json();
