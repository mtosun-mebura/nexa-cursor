...
@foreach($sectionOrder as $sectionKey)
    ...
    @if($isComponent && (($component && view()->exists($component->view ?? '')) || $sectionKey === 'component:nexa.recente_vacatures' || $sectionKey === 'component:taxi.tarieven' || $sectionKey === 'component:taxi.boekingsmodule' || $sectionKey === 'component:taxi.boekingsmodule_v2' || $sectionKey === 'component:taxi.algemene_boekingsmodule' || $sectionKey === 'component:website.google_reviews' || $sectionKey === 'component:nexa.google_reviews' || $sectionKey === 'component:website.nexa_modules_overview'))
        ...
        @elseif($sectionKey === 'component:taxi.boekingsmodule_v2' && view()->exists('frontend.website.components.nexataxi-boekingsmodule-v2'))
            @include('frontend.website.components.nexataxi-boekingsmodule-v2', ['homeSections' => $homeSections ?? [], 'sectionKey' => $sectionKey, 'page' => $page ?? null, 'googleMapsApiKey' => $googleMapsApiKey ?? ''])
        @elseif($sectionKey === 'component:taxi.algemene_boekingsmodule' && view()->exists('frontend.website.components.nexataxi-algemene-boekingsmodule'))
            @include('frontend.website.components.nexataxi-algemene-boekingsmodule', ['homeSections' => $homeSections ?? [], 'sectionKey' => $sectionKey, 'page' => $page ?? null, 'googleMapsApiKey' => $googleMapsApiKey ?? '', 'websitePageCompanyId' => isset($page) && $page->company_id ? (int) $page->company_id : null])
    ...
    <section class="pt-8 md:pt-10 pb-12 md:pb-16 {{ $whyBg['surface_class'] }} {{ $whyBg['wrapper_class'] }}" @if($whyBg['color_style'] !== '') style="{{ $whyBg['color_style'] }}" @endif>
        ...
        <div class="max-w-screen-xl px-6 sm:px-8 lg:px-16 mx-auto relative z-10">
            <div class="max-w-3xl mx-auto text-center">
                @if($v('_title'))
                <h2 class="text-3xl sm:text-4xl lg:text-5xl font-bold {{ $whyBg['title_color_style'] === '' ? 'text-gray-900 dark:text-white' : '' }}" @if($whyBg['title_color_style'] !== '') style="{{ $whyBg['title_color_style'] }}" @endif>
    ...
    <section id="next-landing-vpn-cta" class="py-16 lg:py-24 relative overflow-hidden" style="background-color: {{ $primaryColor }}; @if($ctaBgUrl) background-image: url({{ $ctaBgUrl }}); background-size: cover; background-blend-mode: multiply; @endif">
        <div class="max-w-screen-xl px-6 sm:px-8 lg:px-16 mx-auto relative z-10 text-center">
            ...
            <div class="mt-8 flex flex-col sm:flex-row justify-center gap-4 pt-5">
                ...
                @endphp
                <a href="{{ $url($sectionData['cta_secondary_url'] ?? '/jobs') }}" class="inline-flex justify-center items-center px-8 py-4 text-base font-bold border-2 border-white rounded-lg text-white transition-all duration-200 hover:bg-white/20 hover:shadow-xl hover:-translate-y-1{{ $ctaSecondaryHoverCss !== '' ? ' nexa-cta-btn--custom-hover' : '' }}" @if($ctaSecondaryHoverCss !== '') style="{{ $ctaSecondaryHoverCss }}" @endif>
                    {{ $sectionData['cta_secondary_text'] }}
                </a>
                @endif
            </div>
