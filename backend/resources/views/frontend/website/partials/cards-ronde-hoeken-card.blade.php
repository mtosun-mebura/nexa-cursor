...
<article class="kt-card w-full shadow-md border border-gray-200 dark:border-gray-600 rounded-xl overflow-hidden flex flex-col {{ $imageBgColor !== '' ? '' : 'bg-white dark:bg-gray-800' }}" style="min-height: 0;@if($imageBgColor !== '') background-color: {{ $imageBgColor }};@endif">
    ...
    <div class="website-image-carousel-media cards-ronde-hoeken-image-wrap flex-shrink-0 overflow-hidden"@if($imageBgColor !== '') style="background-color: {{ $imageBgColor }};"@endif>
        <img alt="" class="website-image-carousel-fit cards-ronde-hoeken-image" src="{{ $imgUrlBust }}" loading="lazy" decoding="async">
    </div>
    @endif
