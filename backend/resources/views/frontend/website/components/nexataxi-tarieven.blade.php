<section class="w-full pt-12 md:pt-16" aria-labelledby="nexataxi-tarieven-heading">
    ...
    @if($hasRates || $hasSectionItemsWithContent)
        @if(!empty($sectionItems))
            ...
            <div class="nexataxi-pricing my-10 md:my-14" data-nexataxi-pricing data-price-animation="{{ $priceAnimation ? '1' : '0' }}">
                <div class="flex flex-wrap justify-center items-stretch gap-6 lg:gap-8 mb-6 lg:mb-8">
                    @foreach($sectionItems as $itemIndex => $item)
                        ...
                        @endphp
                        @if($isOverigeKosten)
                        <div class="{{ $wrapperClass }}" style="{{ $wrapperStyle }}" data-nexataxi-card-wrapper data-card-size="{{ $cardSize }}">
                        <div class="rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 transition-all duration-700 ease-out shadow-lg w-full h-full flex flex-col opacity-0 translate-y-4 {{ $textAlignClass }} transition-transform duration-200 hover:-translate-y-2" data-nexataxi-card data-card-size="{{ $cardSize }}" data-card-index="{{ $itemIndex }}">
                        @if($imageUrl)
                            <div class="nexataxi-card-image-wrap aspect-[16/10] w-full overflow-hidden shrink-0 relative" style="background-color: {{ $imageBgColor }};">
                                <div class="nexataxi-card-image-loader absolute inset-0 flex items-center justify-center transition-opacity duration-300 bg-gray-100/90 dark:bg-gray-800/90" aria-hidden="true">
                                ...
                                <img src="{{ $imageUrl }}" alt="{{ e($title) }}" class="nexataxi-card-image w-full h-full object-cover object-center opacity-0 transition-opacity ease-out" style="transition-duration: {{ $imageFadeDuration }}ms;" loading="lazy" onload="this.closest('.nexataxi-card-image-wrap').classList.add('image-loaded')" onerror="this.closest('.nexataxi-card-image-wrap').classList.add('image-loaded')">
                            </div>
                            @endif
            ...
            (function() {
                function syncTaxiroyaalCardHeights(root) {
                    var cards = Array.prototype.slice.call(root.querySelectorAll('[data-nexataxi-card]'));
                ...
                function run() {
                    var roots = document.querySelectorAll('[data-nexataxi-pricing]');
