...
@section('title', 'Vestiging Bewerken')
    ...
    <form action="{{ route('admin.companies.locations.update', [$company, $location]) }}" method="POST">
        ...
        <div class="grid gap-5 lg:gap-7.5">
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" 
                                       ...
                                       name="house_number" 
                                       value="{{ old('house_number', $location->house_number) }}"
                                       style="width: 12ch;"
                                       required>
                                @error('house_number')
                                    <div class="text-xs text-destructive mt-1">{{ $message }}</div>
                                    ...
                                    <div class="flex-1 flex flex-col gap-3">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" 
                                                   ...
                                                   name="country" 
                                                   value="{{ old('country', $location->country ?? 'Nederland') }}"
                                                   readonly>
                                            @error('country')
                                                <div class="text-xs text-destructive mt-1">{{ $message }}</div>
                                            @enderror
            ...
            <div class="flex items-center justify-end gap-2.5">
                <a href="{{ route('admin.companies.locations.show', [$company, $location]) }}" class="kt-btn kt-btn-outline">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initGoogleMap(lat, lng) {
        ...
        if (mapElement.offsetWidth === 0 || mapElement.offsetHeight === 0) {
            ...
            mapElement.style.height = '208px';
            mapElement.style.minHeight = '208px';
        }
        ...
        try {
            ...
            if (lat && lng) {
                ...
                const streetAddress = @json(trim(($location->street ?? '') . ' ' . ($location->house_number ?? '') . ($location->house_number_extension ? '-' . $location->house_number_extension : '')));
                const postalCity = @json(trim(($location->postal_code ?? '') . ' ' . ($location->city ?? '')));
    ...
    function waitForGoogleMaps() {
        if (typeof google !== 'undefined' && typeof google.maps !== 'undefined') {
            ...
            @endphp
            initGoogleMap({{ $lat ?: 'null' }}, {{ $lng ?: 'null' }});
        } else {
