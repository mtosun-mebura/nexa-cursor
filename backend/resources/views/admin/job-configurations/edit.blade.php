...
@section('title', 'Job Configuratie Bewerken')
    ...
    <div class="flex flex-col gap-5 pb-7.5">
        <div class="flex flex-wrap items-center justify-between gap-5">
            <h1 class="text-xl font-medium leading-none text-mono">
                Job Configuratie Bewerken
            </h1>
        </div>
        <div class="flex items-center">
            <a href="{{ route('admin.job-configurations.show', $jobConfiguration) }}" class="kt-btn kt-btn-outline">
                ...
                <div class="kt-card-table kt-scrollable-x-auto pb-3">
                    <table class="kt-table kt-table-border-dashed align-middle text-sm text-muted-foreground">
                            ...
                            <td class="min-w-48 w-full">
                                <select class="kt-select @error('type_id') border-destructive @enderror" name="type_id" data-kt-select="true" required>
                                    ...
                                    @foreach($types ?? [] as $type)
                                        <option value="{{ $type->id }}" {{ old('type_id', $jobConfiguration->type_id) == $type->id ? 'selected' : '' }}>
                                            {{ $type->display_name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" class="kt-input @error('value') border-destructive @enderror" name="value" value="{{ old('value', $jobConfiguration->value) }}" required maxlength="100" placeholder="Bijv. Fulltime, 08:00-16:00, Open">
                            ...
                            <td>
                                <select class="kt-select @error('company_id') border-destructive @enderror" name="company_id">
                                    ...
                                    @foreach($companies as $company)
                                        <option value="{{ $company->id }}" {{ old('company_id', $jobConfiguration->company_id) == $company->id ? 'selected' : '' }}>
                                            {{ $company->name }}
                                        </option>
                                    @endforeach
                                </select>
