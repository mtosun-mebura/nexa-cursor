                    ...
                    <table class="kt-table kt-table-border-dashed align-middle text-sm text-muted-foreground">
                        <tr>
                            ...
                            <td class="min-w-48 w-full">
                                <input type="text" class="kt-input @error('name') border-destructive @enderror" name="name" value="{{ old('name') }}" required>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" class="kt-input @error('slug') border-destructive @enderror" name="slug" value="{{ old('slug') }}" placeholder="Automatisch gegenereerd">
                                <div class="text-xs text-muted-foreground mt-1">Laat leeg om automatisch te genereren op basis van de naam</div>
                                @error('slug')<div class="text-xs text-destructive mt-1">{{ $message }}</div>@enderror
                            </td>
                        </tr>
                        <tr>
                            <td class="text-secondary-foreground font-normal align-top">Beschrijving</td>
                            <td>
                                <textarea class="kt-input pt-1 @error('description') border-destructive @enderror" name="description" rows="4" placeholder="Beschrijving...">{{ old('description') }}</textarea>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="flex items-center gap-3">
                                    <input type="color" class="kt-input @error('color') border-destructive @enderror" name="color" value="{{ old('color', '#007bff') }}" style="width: 60px; height: 40px; padding: 0;">
                                    <span class="text-xs text-muted-foreground">{{ old('color', '#007bff') }}</span>
                                </div>
                                @error('color')<div class="text-xs text-destructive mt-1">{{ $message }}</div>@enderror
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                ...
                                @error('icon')<div class="text-xs text-destructive mt-1">{{ $message }}</div>@enderror
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <label class="kt-label flex items-center">
                                    <input type="checkbox" class="kt-switch kt-switch-sm" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }}>
...
<!-- Icon Picker Modal (same as edit) -->
@php
    $heroicons = collect();
    try {
...
document.addEventListener('DOMContentLoaded', function () {
    // Auto-generate slug from name (same as edit)
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    modal.addEventListener('click', function (e) {
        ...
        if (svg) {
            ...
            if (cloned) cloned.classList.add('w-5', 'h-5', 'text-muted-foreground');
        }
