                    ...
                    <table class="kt-table kt-table-border-dashed align-middle text-sm text-muted-foreground">
                                ...
                                <select class="kt-select @error('location_id') border-destructive @enderror" 
                                    ...
                                    @if($company)
                                        ...
                                        @if($mainLocation && !$hasMainLocationInList)
                                            ...
                                            @endphp
                                            <option value="{{ $mainLocation->id }}" {{ old('location_id', $notification->location_id) == $mainLocation->id ? 'selected' : '' }}>
                                                {{ $mainDisplayName }}
                                            </option>
                                        @elseif(!$mainLocation && $hasCompanyAddress)
                                            ...
                                            @endphp
                                            <option value="company_main" {{ old('location_id', $notification->location_id) == 0 || old('location_id', $notification->location_id) == 'company_main' ? 'selected' : '' }}>
                                                {{ $companyDisplayName }}
                                            </option>
                                        @endif
                                    @endif
                                    @if($companyLocations->count() > 0)
                                        @foreach($companyLocations as $location)
                                            ...
                                            @endphp
                                            <option value="{{ $location->id }}" {{ old('location_id', $notification->location_id) == $location->id ? 'selected' : '' }}>
                                                {{ $locationDisplayName }}
                                            </option>
                                        @endforeach
                                    @endif
                                    ...
                                    @php
                                        $isRemoteSelected = (old('location_id', $notification->location_id) === 'remote' || 
                                                             old('location_id', $notification->location_id) === '-1' ||
                                                             old('location_id', $notification->location_id) === -1 ||
                        ...
                        <tr>
                            ...
                            <td>
                                @if($notification->file_path)
                                    <div class="mb-2">
                                        <a href="{{ \Storage::url($notification->file_path) }}" target="_blank" class="text-sm text-primary hover:underline">
                                            <i class="ki-filled ki-file me-1"></i>
                                ...
                                @enderror
                                <div class="text-xs text-muted-foreground mt-1">
                                    Optioneel: upload een nieuw bestand om het huidige te vervangen (max 10MB)
                                </div>
                            </td>
                        </tr>
                        <tr>
