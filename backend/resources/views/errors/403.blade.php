...
<head>
    ...
    <title>403 — Toegang geweigerd | {{ config('app.name') }}</title>
    <style>
        *,
        *::before,
        *::after {
        ...
        .error403-toolbar a,
        .error403-toolbar .error403-actions {
            pointer-events: auto;
        }
        ...
        .error403-actions .btn-outline {
            background: rgba(255, 255, 255, 0.08);
            color: #fff;
            border: 1px solid rgba(255, 255, 255, 0.25);
        }

        .error403-actions .btn-primary {
            background: #fff;
            color: #111;
...
<body>
    @php
        ...
        if (! auth()->check()) {
            $backUrl = \Illuminate\Support\Facades\Route::has('home') ? route('home') : url('/');
        } else {
            ...
            if (str_contains($currentUrl, '/admin/vacancies/') || str_contains($currentUrl, '/admin/skillmatching/vacancies/')) {
                ...
                if ($user->hasRole('super-admin') || $user->can('view-categories')) {
                    $backUrl = route('admin.categories.index');
                }
            } elseif (str_contains($currentUrl, '/admin/notifications/')) {
                if ($user->hasRole('super-admin') || $user->can('view-notifications')) {
                ...
                if ($user->hasRole('super-admin') || $user->can('view-matches') || $user->can('skillmatching.matches.view')) {
                ...
                if ($user->hasRole('super-admin') || $user->can('view-interviews') || $user->can('skillmatching.interviews.view')) {
    ...
    <div id="error403-root" aria-label="Klik om animatie te herhalen">
        <svg
            class="text"
    ...
    <footer class="error403-toolbar" role="contentinfo">
        <p>
            <strong>403 — Toegang geweigerd</strong><br>
    ...
    <script>
        (function () {
            const root = document.getElementById('error403-root');
            if (!root) {
                return;
            }
