...
SOURCE = ROOT / "backend/docs/NEXA-SAAS-MACH-MICROSERVICES-PLAN.md"
OUT = ROOT / "backend/docs/NEXA-SAAS-MACH-MICROSERVICES-PLAN.pdf"
...
class Renderer:
    ...
    def new_page(self):
        ...
        self.y = MARGIN_TOP
        self.draw.rectangle((0, 0, PAGE_W, 28), fill="#0f766e")
        self.draw.text((MARGIN_X, PAGE_H - 54), f"NEXA SaaS MACH microservices plan - pagina {self.page_no}", font=F_SMALL, fill="#64748b")
    ...
    def paragraph(self, text: str, indent: int = 0, bullet: str | None = None):
        ...
        if bullet:
            ...
            self.draw.text((x, self.y), bullet, font=F_BODY_BOLD, fill="#0f766e")
    ...
    def h2(self, text: str):
        ...
        self.draw.text((MARGIN_X, self.y), text, font=F_H2, fill="#0f766e")
        self.y += 42
    ...
    def table(self, rows: list[list[str]]):
        ...
        cols = len(rows[0])
        if cols == 2:
            widths = [330, BODY_W - 330]
        elif cols == 3:
            widths = [250, 360, BODY_W - 610]
        else:
        ...
        for r, (wrapped, row_h) in enumerate(wrapped_rows):
            if self.y + row_h > PAGE_H - MARGIN_BOTTOM:
                self.new_page()
            x = x0
            fill = "#e0f2fe" if r == 0 else ("#ffffff" if r % 2 else "#f8fafc")
    ...
    def title_page(self):
        ...
        self.draw.rectangle((0, 0, PAGE_W, 260), fill="#0f766e")
        self.draw.text((MARGIN_X, 96), "NEXA SaaS", font=F_TITLE, fill="#ffffff")
        self.draw.text((MARGIN_X, 158), "MACH microservices migratieplan", font=font(38, True), fill="#d1fae5")
        rounded_rect(self.draw, (MARGIN_X, 330, PAGE_W - MARGIN_X, 570), 28, "#ffffff", "#cbd5e1", 2)
        intro = (
            "Een gefaseerd plan om de huidige modulaire Laravel SaaS-applicatie om te zetten "
            "naar een MACH-architectuur met microservices, API-first contracten, cloud-native "
            "containers en headless clients."
        )
        y = 380
        for line in wrap_to_width(self.draw, intro, F_BODY, BODY_W - 80):
            self.draw.text((MARGIN_X + 40, y), line, font=F_BODY, fill="#334155")
            y += 34
        self.draw.text((MARGIN_X + 40, 510), "Datum: 2026-06-03", font=F_BODY_BOLD, fill="#0f172a")
        self.y = 650
